...
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Insight and xpchaind? Insight is configured for:';
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          if (self.blockChainHeight === blockInfo.height ||
            ...
            oldtip = tip;
            if (!tip)
              throw new Error('Previous blockchain tip was not found on xpchaind. Please reset Insight DB. Tip was:' + tip)
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  info('Got ' + self.height +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at xpchaind');

  if (self.blockExtractor) {
    info('xpchaind dataDir configured...importing blocks from .dat files');

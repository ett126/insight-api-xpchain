...
Rpc.getBlock = function(hash, cb) {
  ...
  bitcoreRpc.getBlock(hash, function(err,info) {
    ...
    var txId = info.result.tx[0]
    bitcoreRpc.getRawTransaction(txId, 1, function(err, txInfo) {
        // Not found?
        if (err && err.code === -5) return cb();
        if (err) return cb(self.errMsg(err));
        
        var txinfo = self._parseTxResult(txInfo.result);
        if(txinfo.isCoinBase){
          info.result.reward = txinfo.valueOut;
        }else{
          console.log('not coinbase tx');
        }
